...
from simplecode.config import ConfigError, load_config
from simplecode.hooks import HookConfigError, HookEngine, load_hooks
from simplecode.permissions import PermissionMode


def main() -> None:
    # 先确保 .simplecode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".simplecode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".simplecode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="simplecode", description="SimpleCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from simplecode.remote import RemoteServer
    ...
    from simplecode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from simplecode.client import create_client, resolve_context_window
    from simplecode.conversation import ConversationManager
    from simplecode.memory.instructions import load_instructions
    ...
    from simplecode.worktree import WorktreeManager
