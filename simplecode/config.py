...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".simplecode" / "config.yaml",
        cwd / ".simplecode" / "config.yaml",
        cwd / ".simplecode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .simplecode/config.yaml "
            "in project or ~/.simplecode/config.yaml"
        )
    return merged
