...
from simplecode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from simplecode.commands.completion import CompletionPopup
from simplecode.commands.handlers import register_all_commands
from simplecode.config import MCPServerConfig, ProviderConfig
from simplecode.hooks import HookContext, HookEngine, load_hooks
from simplecode.conversation import ConversationManager, Message
from simplecode.mcp import ConnectResult, MCPManager
...
from simplecode.agents.task_manager import TaskManager
from simplecode.agents.trace import TraceManager
from simplecode.agents.notification import inject_task_notifications
from simplecode.commands.handlers.tasks import create_tasks_command
from simplecode.skills.executor import SkillExecutor
from simplecode.skills.loader import SkillLoader
from simplecode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from simplecode.cache import FileCache
from simplecode.tools import ToolRegistry, create_default_registry
from simplecode.tools.agent_tool import AgentTool
from simplecode.tools.ask_user import AskUserEvent, AskUserTool
from simplecode.tools.impl.tool_search import ToolSearchTool
from simplecode.tools.load_skill import LoadSkill
from simplecode.worktree.cleanup import start_stale_cleanup_task
from simplecode.worktree.manager import WorktreeManager
from simplecode.commands.handlers.worktree import create_worktree_command
from simplecode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".simplecode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".simplecode" / "history"
...
class SimpleCodeApp(App):
    ...
    def __init__(
        ...
        from simplecode.config import SandboxAppConfig
    ...
    def on_mount(self) -> None:
        self.register_theme(_SIMPLECODE_THEME)
        self.theme = "simplecode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".simplecode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".simplecode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".simplecode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from simplecode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.simplecode/config.yaml",
                        f"{work_dir}/.simplecode/permissions.local.yaml",
                    ],
        ...
        from simplecode.filehistory import FileHistory
        ...
        from simplecode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from simplecode.config import WorktreeConfig
        ...
        from simplecode.tools.enter_worktree import EnterWorktreeTool
        from simplecode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from simplecode.teams.manager import TeamManager
        from simplecode.tools.team_create import TeamCreateTool
        from simplecode.tools.team_delete import TeamDeleteTool
        ...
        from simplecode.commands.handlers.trace import create_trace_command
        ...
        from simplecode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from simplecode.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from simplecode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from simplecode.plan_dialog import InlinePlanWidget, PlanChoice
        from simplecode.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from simplecode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from simplecode.permission_dialog import InlinePermissionWidget
