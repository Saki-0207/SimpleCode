...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.simplecode/SIMPLECODE.md, ~/.simplecode/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 SIMPLECODE.md 和 AGENTS.md
    3. workDir/.simplecode/INSTRUCTIONS.md（遗留格式）
    4. workDir/SIMPLECODE.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".simplecode" / "SIMPLECODE.md")
    _add(home / ".simplecode" / "AGENTS.md")

    # 2. 项目目录链
    for d in _project_instruction_dirs(root):
        _add(d / "SIMPLECODE.md")
        _add(d / "AGENTS.md")

    # 3. 遗留格式
    _add(root / ".simplecode" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "SIMPLECODE.local.md")
