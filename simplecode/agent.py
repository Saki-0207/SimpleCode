...
from simplecode.conversation import ThinkingBlock as ConvThinkingBlock
...
from simplecode.hooks import HookContext, HookEngine, ToolRejectedError
from simplecode.hooks.engine import HookNotification
from simplecode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".simplecode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".simplecode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from simplecode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from simplecode.context.manager import (
