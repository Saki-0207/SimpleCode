...
from simplecode.worktree.models import Worktree, WorktreeSession
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".simplecode" / "worktrees"
        )
        self._simplecode_dir = Path(repo_root) / ".simplecode"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._simplecode_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._simplecode_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._simplecode_dir)
        if session is None:
            return None
