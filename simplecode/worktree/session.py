...
def _session_path(simplecode_dir: Path) -> Path:
    return simplecode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(simplecode_dir)
    if session is None:
...
def load_worktree_session(simplecode_dir: Path) -> WorktreeSession | None:
    path = _session_path(simplecode_dir)
    if not path.exists():
        return None
    try:
